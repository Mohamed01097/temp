...
class AccountCommonReport(models.TransientModel):
    ...
    with_posted = fields.Boolean(string='المرحلة فقط')
    ...
    def _build_contexts(self, data):
        ...
        result['with_posted'] = data['form']['with_total'] or False
    ...
    @api.onchange('with_posted')
    def only_posted(self):
        if self.with_posted:
            self.target_move = 'posted'
        if not self.with_posted:
    ...
    def check_report(self):
        ...
        if not self.tag_ids and not self.partner_ids and not self.with_total:
            raise ValidationError(_("You must select at least tags or partners"))
        self.ensure_one()
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(
            ['date_from', 'with_total','with_posted','with_zero_balance','with_old_balance', 'tag_ids', 'date_to', 'journal_ids', 'target_move',
             'company_id'])[0]
